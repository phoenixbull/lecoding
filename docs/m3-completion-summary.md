...
零缓存类型检查是必测项：本机此前多次出现的「本地 25/25 通过但远端 CI 失败」正是 turbo 缓存掩盖了真实诊断（见 §3）。候选 `d64fbf2` 已推送，CI 运行 #4 判定 passing，本机与远端首次取得同一 SHA 的一致绿灯。
...
- **退出条件已满足（2026-09-07）**：候选 `d64fbf2` 已推送至 `origin/master`，CI 运行 #4 结果 **passing**（workflow `ci`，约 2m29s）。判定依据为 badge 端点 `/actions/workflows/ci.yml/badge.svg`，以及运行列表对该 SHA 的状态。此为「同一候选 SHA 的 Linux CI 全绿」的首次有效记录。
...
**本次 RC 的准入标准（2026-09-07 确认）**：固定 12 项 acceptance 黄金任务**全部通过**（12/12）。

现有 live 测试只断言任务数量与 token 非零，**不断言通过率**，因此需要在公开评估结果 seam 上新增报告门禁，而不是只看测试是否报错。该标准附三条限定，缺一不可：

1. **保留全部执行轮次**：归档每一次尝试的结果与日志，不删除、不覆盖。
2. **失败即修复并重跑完整集合**：不允许拼接多轮的成功结果凑成 12/12——那等于用选择性归档伪造通过。
3. **12/12 的语义边界**：它只证明「该固定验收集在本次候选上通过」，**不代表生产成功率为 100%**。acceptance 集合是抽样，不是全量等价证明；不得据此宣称「模型在生产中不会失败」。

成本/时长预算须在执行**之前**记录，不采用旧文档价格冒充当前账单。

### 4.3 证据索引

已建立索引规范 [`docs/evidence/m3/README.md`](evidence/m3/README.md)；待固定 RC SHA 后按 `docs/evidence/m3/<candidate>/` 建立，逐项记录：SHA/tag、执行人、日期、OS/架构、产物摘要、环境版本、命令/退出码、结果、跳过原因、原始日志链接。日志需脱敏，数据库备份不入库。
...
## 6.1 Release notes 草稿（待固定 RC 后定稿）

**本版本包含**

- 桌面客户端：设备绑定、共享 Run 控制台、OS 原生凭据存储、安装包构建链路。
- 本地执行（Phase 4B）：桌面可在本机执行 Run，服务端 RunEngine、审批、事件与验证契约不分叉。
  - 经认证的 WSS 传输：断线重连、命令幂等去重、事件游标续传、设备撤销即断。
  - 三档文件访问权限由 OS 沙箱在进程创建时强制（macOS 内核级；Windows best-effort）。
  - 跨重启恢复：durable journal、未知结果命令不重放、keep/discard 幂等、清理残留可观测。
- 更新链路：清单 SHA-256 + Ed25519 签名校验，密钥由桌面配置钉住。

**升级步骤**

1. 备份（如使用服务端部署）：PostgreSQL 与 Artifact，记录一致性时间点。
2. 安装与当前平台/架构匹配的产物；macOS 用 DMG/ZIP，Windows 用 Squirrel 安装包。
3. 首次启动后确认设备凭据仍在（OS 钥匙串）；若已失效，重新绑定设备。
4. 确认本地执行的隔离提示按预期显示（Windows 会声明无内核级文件隔离）。

**回滚步骤**

更新校验器**拒绝降级**，因此不能靠「安装旧版本」回滚自动更新留下的状态——绕过校验器等于撤销这道安全门。回滚应采用：

1. **受控重装**：卸载当前版本，安装此前固定 SHA 的产物。
2. **数据恢复**：按 [operations-runbook.md](operations-runbook.md) 从备份恢复（优先恢复到新建的旧版库再切换，不能假定新 schema 可被旧代码安全读取）。
3. **本地状态清理**：若降级后本地执行状态不兼容，删除本地 Runner 目录（journal、grants、worktrees）并重新绑定——这些是缓存语义，不持有不可替代数据。

**兼容矩阵（当前已知）**

| 平台 | 架构 | 本地文件访问强制等级 | 备注 |
|---|---|---|---|
| macOS | arm64 / x64 | `kernel`（Seatbelt） | 启动探针失败即失败关闭 |
| Windows | x64 | `argv_fence`（best-effort）+ 进程树终止 | 无 Job Object / AppContainer |
| Linux | — | 拒绝本地执行 | 不静默降级为无隔离运行 |

**发布判定**：当前不满足（见 §7），仅交付 RC 与未完成清单。

## 7. 逐项发布检查清单

| 项 | 状态 |
|---|---|
| 同 SHA 类型检查 25/25（零缓存） | ✅ 本机通过；CI 运行 #4 已确认 |
| 同 SHA 全量测试 0 失败 | ✅ 本机通过（1200 通过 / 0 失败）；CI 同 SHA 已通过 |
...
| release notes / 已知限制 / 兼容矩阵 / 回滚步骤 | ✅ 草稿见 §6.1 与 §6，待固定 RC 后定稿 |
