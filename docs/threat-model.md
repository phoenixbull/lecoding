...
| Run input, steering, or model argv contains a deployment credential | RunEngine compares every persistence-boundary string with the deployment-seeded redactor and rejects credential-bearing original/user/tool input before any Run, mailbox, approval, or tool-ledger write |
| Verbose command output exhausts Worker memory or PostgreSQL | Docker pipe capture is byte-bounded per stream; Run/model/tool-ledger state receives only a valid UTF-8 prefix, while the redacted retained overflow is content-addressed outside PostgreSQL |
| User enumerates another Run's retained output | Artifact API paths include the owning Run and verify metadata Run/project identity plus current membership before hash-verifying the local file; failures share one not-found surface |
| Retained output outlives policy | Startup-plus-daily cleanup removes bytes before metadata after seven days and reports deletion counts, failures, and residual storage keys without logging content |
