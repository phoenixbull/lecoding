...
| Electron + React shell (Windows + macOS) | Complete (code path); target-platform GUI smoke pending | Wave 3 | [`apps/desktop/src/main/electron.ts`](file:///Users/letv_lzb/Documents/LeCodex/apps/desktop/src/main/electron.ts) — the production bootstrap that previously did not exist, so the packaged app could not start at all. `main/index.ts` now mounts the real preload (it was `preload: undefined`), registers typed IPC channels, and holds SSE in [`stream-broker.ts`](file:///Users/letv_lzb/Documents/LeCodex/apps/desktop/src/main/stream-broker.ts) because the Renderer CSP is `connect-src 'self'` and credentials must never reach it. `shared/ipc-contract.ts` defines the push-channel whitelist; preload exposes only named subscription functions, never `ipcRenderer.on` itself. The React 19 + Vite renderer renders immutable state through `useSyncExternalStore` and owns no Run semantics. 193 desktop tests pass; 4 target-artifact tests remain environment-gated. |
...
- `pnpm test`: **817 passed + 9 skipped + 0 failed** across 107 files
  (103 passed, 4 skipped). Every skip is an explicit environment gate —
...
| `apps/desktop/test` | 193 passed / 4 skipped |
| `apps/desktop/test/run-loop.integration.test.ts` (deep business E2E) | 4/4 |
