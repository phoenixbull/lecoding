...
| Approval flow | Pending | Phase 1 approve-once/reject-once exists; Phase 2 network/high-risk scopes and durable audit remain. |
...
- Non-Docker suite: 251/251 tests passed across 57 test files; the live model
  baseline remains explicitly opt-in.
- Raw `pnpm test`: 256 tests passed, one opt-in test skipped, and the two
