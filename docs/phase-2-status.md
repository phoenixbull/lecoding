...
| Approval flow | In progress | OpenAI Responses and Chat Completions expose an explicit HTTPS endpoint request. Network and command capabilities can be approved/rejected once, for the exact Run fingerprint, or as an exact project rule by an administrator, then resume through provider continuation with attributed decisions. Project-rule versions are PostgreSQL-backed, independently revocable through admin API/SDK/Web settings, and hidden from non-admin callers. The Web card shows capability, risk, policy reason, exact target, and bounded scope. Edit-and-allow remains. |
| `auto_review` and `full_access` | In progress | `auto_review` now uses an independently injected reviewer with validated `allow`/`ask` output and an immutable PostgreSQL audit record; deterministic project/fixed rules run before it. `full_access` is project-admin-only and server Runs remain workspace-only. Network IP/loopback/private/metadata targets, credential/browser files, Docker socket, and host-control commands are fixed deny across all modes. A dedicated adversarial evaluator covers command-shape and fixed-deny bypass attempts; the remaining V3 fixed-deny and exfiltration matrix still has to close before this row is complete. |
...
- Current focused approval/policy/API/SDK/Web suite: 93/93 tests passed across
  eight test files, including PostgreSQL rule versioning and reviewer audit.
- `pnpm typecheck`: 12/12 workspace tasks passed.
- Raw repository suite: 282 tests passed and one opt-in live-model test was
  ...
  target-Linux/Phase 0 evidence gap, not an assertion regression. HTTP transport
  tests passed 8/8 with local loopback-bind permission.
