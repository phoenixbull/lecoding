...
- `POST /api/v1/runs/:runId/commands` accepts cancel; approve/reject with `once`, exact `run`, or administrator-only exact `project` scope; bounded `edit_approve`; a matching `answer`; or a bounded `steer`. Edit-and-allow is always single-call: command argv may only lose ordered entries and must pass independent review, while an HTTPS endpoint may only move to a strict subdomain on the same port. The audit keeps both normalized capability versions. Answers carry the displayed request ID so a stale tab cannot answer a newer question. Every answer and steer also carries a client-stable `commandId` of at most 128 characters; the SDK generates a UUID unless the caller supplies one for an explicit retry. Steering resolves the current `waiting_user` question or enters the durable mailbox while a Run is queued, preparing, running, or awaiting environment recovery.
