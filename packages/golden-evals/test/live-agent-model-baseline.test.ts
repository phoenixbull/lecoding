...
describe("live OpenAI-compatible golden baseline", () => {
  it.skipIf(!runLiveBaseline)(
    ...
    async () => {
      ...
      const workingRoot = await mkdtemp(join(tmpdir(), "lecoding-live-golden-"));
      const executor = createAgentModelGoldenTaskExecutor({
        workingRoot,
        modelId: config.model,
        pricing,
        createModel: (onUsage) =>
          createOpenAiCompatibleAgentModel({ config, onUsage }),
      ...
      const outputPath = resolve(
        process.env.LECODING_GOLDEN_REPORT ??
          ".artifacts/golden-evals/latest-report.json"
      );
